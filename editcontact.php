...
if(isset($_POST["submit"])){
  ...
  if(empty($Name)){
    ...
    $EditFromURL = $_REQUEST['Edit'];
    global $connection;
    $Query = "UPDATE `address-book`.`contacts` SET `name`='$Name', `surname`='$Surname', `email`='$Email', `address`='$Address',`datetime`='$DateTime' WHERE `id`='$EditFromURL'";
  ...
  <!-- <div class="container"> -->
    <div class="row">
      ...
      <div class="col-sm-10">
        ...
        <div>
        <?php 
        $SearchQueryParameter = $_REQUEST['Edit'];
        $connection;
        $Query = "SELECT * FROM `address-book`.`contacts` WHERE `id`='$SearchQueryParameter'";
        $Execute = mysqli_query($connection, $Query);
        while($DataRows=mysqli_fetch_array($Execute)){
            $NameToBeUpdated = $DataRows["name"];
            $SurnameToBeUpdated = $DataRows["surname"];
            $EmailToBeUpdated = $DataRows["email"];
            $AddressToBeUpdated = $DataRows["address"];
        }
        ?>
          <form action="editcontact.php?Edit=<?php echo $SearchQueryParameter; ?>" method="post">
          <label for="name" class="font-weight-bold">Name:</label>
            <input type="text" id="name" name="Name" class="form-control mt-1" placeholder="Enter Name" value="<?php echo $NameToBeUpdated; ?>" required>
            <label for="surname" class="font-weight-bold">Surname:</label>
            <input type="text" id="surname" name="Surname" class="form-control mt-1" placeholder="Enter Surname" value="<?php echo $SurnameToBeUpdated; ?>" required>
            <label for="email" class="font-weight-bold">Email:</label>
            <input type="text" id="email" name="Email" class="form-control mt-1" placeholder="Enter Email" value="<?php echo $EmailToBeUpdated; ?>" required>     

            <label for="address" class="font-weight-bold">Address:</label>
            <textarea class="form-control mt-1" name="Address" id="address"><?php echo $AddressToBeUpdated; ?></textarea><br />

            <input class="btn btn-success btn-primary btn-block mt-1" type="submit" name="submit" value="Update Contact">
          </form>
        </div>
